...
class CanvasClient:
    def __init__(self, base_url: str, access_token: str, session: Optional[ClientSession] = None) -> None:
        ...
        self._token = access_token.strip() if access_token else access_token
    ...
    async def _get_all_pages(self, path: str, params: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
        ...
        while True:
            ...
            async with self._session.get(url, headers=self._headers, params=params) as resp:
                ...
                if resp.status >= 400:
                    txt = await resp.text()
                    _LOGGER.error("Canvas error %s @ %s: %s", resp.status, self._base, txt)
                    raise CanvasApiError(f"{resp.status}: {txt}")
    ...
    async def list_courses(self) -> List[Dict[str, Any]]:
        params = {"enrollment_state": "active", "include[]": ["term"], "per_page": 50}
        return await self._get_all_pages(PATH_COURSES, params)
    ...
    async def list_assignments(self, course_id: str, bucket: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        path = PATH_ASSIGNMENTS.format(course_id=course_id)
        return await self._get_all_pages(path, params)

    async def get_submission_self(self, course_id: str, assignment_id: str) -> Dict[str, Any]:
        path = PATH_SUBMISSIONS_SELF.format(course_id=course_id, assignment_id=assignment_id)
        url = URL(self._base + path)
    ...
    async def list_enrollments(self, course_id: str) -> List[Dict[str, Any]]:
        params = {"type[]": ["StudentEnrollment"], "user_id": "self", "per_page": 50}
        path = PATH_ENROLLMENTS.format(course_id=course_id)
        return await self._get_all_pages(path, params)
